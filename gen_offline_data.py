...
def generate_offline_problems(save_path, start_chunk=0, n_proofs=1e6, n_workers=20, num_proof_per_chunk=1000,
                            proof_length=15, seed=None, max_length_mode=True):
    num_chunks = n_proofs // num_proof_per_chunk
    np.random.seed(seed)
    if seed is not None:
        torch.manual_seed(seed)
    for cur_chunk in tqdm(range(start_chunk, num_chunks)):
        problems = []
...
if __name__ == '__main__':
    generate_offline_problems(save_path='./data', start_chunk=311, n_proofs=1000000, n_workers=60, num_proof_per_chunk=1000,)